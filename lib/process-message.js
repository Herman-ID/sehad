    ...
    .then(responses => {
      ...
      if (result.intent.displayName === "detect-plant") {
        ...
        pusher.trigger("bot",`${channel_id}`,{
          message:"Mohon menunggu <i class='fa fa-spinner fa-spin'></i>"
        })
        var req = {body:{}};
        req.body = {
        ...
        tumbuhan.getTumbuhan(req,function(respone){
          console.dir("origin :" + url.hostname);
          if(respone.data.length > 0){
            scrap.getContentWikipedia(respone.data[0].wikipedia,function(hasil){
              pusher.trigger("bot", `${channel_id}`, {
                message: hasil.image+hasil.message
              });
              pusher.trigger("bot",`${channel_id}`,{
                message:"<a href='"+respone.data[0].wikipedia+"'>"+respone.data[0].wikipedia+"</a>"
              })
            })
          }else{
            pusher.trigger("bot", `${channel_id}`, {
      ...
      if (result.intent.displayName === "detect-manfaat") {
        const manfaat = result.parameters.fields["manfaat"].stringValue;
        return pusher.trigger("bot", `${channel_id}`, {
          message: `manfaat ${manfaat} adalah...`
        });
      }
