...
const processMessage = (message,channel_id) => {
  ...
  sessionClient
    ...
    .then(responses => {
      ...
      console.dir(result.intent.displayName);
        ...
        penyakit_model.getPenyakit(request,function(respone){
          if(respone.length > 0){
            ...
            penyakit_model.getPenyakitObat(request,function(result){
              if(result.length>0){
                var option = "Berikut tanaman herbal yang bisa menjadi alternatif untuk mengobati "+penyakit+"<br><ul>";
                for (let i = 0; i < result.length; i++) {
                    option += "<li>"+result[i].nama+"</li>";
                }
                option += "<ul>";
                var output ={
                  type:"penyakit",
