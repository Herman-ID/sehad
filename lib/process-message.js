...
var jamu_model = require("../models/jamu_model");
...
const processMessage = (message, channel_id) => {
  ...
  sessionClient
    ...
    .then(responses => {
      ...
      //repon untuk pemetaan
      if (result.intent.displayName === "detect-map") {
        const plant = result.parameters.fields["plant"].stringValue;
        var req = { body: {} };
        req.body = {
          keyword: plant.replace('?', '')
        ...
        tumbuhan.getTumbuhan(req, function (respone) {
          if (respone.data.length > 0) {
            ...
            var output = {
              type: "peta",
              status: false,
              message: `Saya belum dimana ${plant} itu.`
            }
      ...
      if(result.intent.displayName === "detect-jamu"){
          const jamu = result.parameters.fields["jamu"].stringValue;
          var req = { body: {} };
          req.body = {
              keyword: jamu.replace('?', '')
          }

          jamu_model.searchJamu(req,function (respon) {
              if(respon.length > 1){
              var output = {
                  type: "jamu",
                  status: true,
              ...
              jamu_model.getDetailJamu({body:{jamu_id:respon[0].id}},function (respone) {
                  var output = {
                      type: "jamu",
                      status: true,
                      isJamu: true,
                ...
                pusher.trigger("bot", `${channel_id}`, {
                    message: `Saya belum mengerti apa itu ${jamu}. Ajarin aku dong.`
                });
            }
          })
          return null;
